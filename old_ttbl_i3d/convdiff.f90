...
!!!original
  ...
  !if (itime.eq.0.OR.mod(itime,imodulo).eq.0) then
  ! write(filename,"('./data/tcx',I4.4)") itime/imodulo
  ! call decomp_2d_write_one(1,td1,filename,2)
  ! write(filename,"('./data/tcy',I4.4)") itime/imodulo
  ! call decomp_2d_write_one(1,te1,filename,2)
  ! write(filename,"('./data/tcz',I4.4)") itime/imodulo
  ! call decomp_2d_write_one(1,tf1,filename,2)
  !endif

  !if(itime.eq.1 .AND. nrank.eq.0)write(*,*)'yp = ',yp(:)

  !FINAL SUM: DIFF TERMS + CONV TERMS + TTBL TRANSPORT TERM
  ta1 = ta1 + td1
  tb1 = tb1 + te1
  tc1 = tc1 + tf1
...
subroutine scalar(ux1,uy1,uz1,phi1,phis1,phiss1,di1,ta1,tb1,tc1,td1,&
  ...
  do is = 1, nphi
     ...
     do ijk = 1,nvect1
        tb1(ijk,1,1) = tb1(ijk,1,1)*ux1(ijk,1,1)
     enddo
     ...
     if (istret.ne.0) then
        do k = 1,ysize(3)
           do j = 1,ysize(2)
              do i = 1,ysize(1)
                 ta2(i,j,k) = ta2(i,j,k)*pp2y(j)-pp4y(j)*tb2(i,j,k)
              enddo
           enddo
        enddo
     endif
     do ijk = 1,nvect2
       tb2(ijk,1,1) = tb2(ijk,1,1)*uy2(ijk,1,1)
     enddo
  ...
  if (nrank == 0) print *, "sgsphi",is,"min max= ",minval(sgsphi1(:,:,:,is)),maxval(sgsphi1(:,:,:,is))
...
#else
     do ijk=1,nvect1
        ta1(ijk,1,1)=(xnu/nsc(is))*ta1(ijk,1,1)-tb1(ijk,1,1) -uy1(ijk,1,1) !solving for temperature fluctuations
     enddo
#endif
     ...
     do j=1,xsize(2)
       if (istret.eq.0) y=real(j+xstart(2)-1-1,mytype)*dy
       if (istret.ne.0) y=yp(j+xstart(2)-1)
        ta1(:,j,:) = ta1(:,j,:) - thetad*y*tb1(:,j,:)!dphi1dy
     enddo
