subroutine parameter()
  ...
  allocate(nsc(nphi),uset(nphi),cp(nphi),ri_b(nphi),group(nphi))
  ...
  if (nrank==0) then
     ...
     write(*,"(' Rossby number Ro : ',F15.8)") ro
     ...
     if (iscalar.eq.1) then
        ...
        do is=1, nphi
           ...
           write (*,"(' Richardson number  : ',F15.8)") ri_b(is)
           !write (*,"(' Settling velocity  : ',F15.8)") uset(is)
           write (*,"(' Prandtl number     : ',F15.8)") nsc(is)
        end do
     endif
