...
USE var
implicit none
!if(itime.eq.ifirst)dt0=dt;dt1=dt;dt2=dt
dt2 = dt1
dt1 = dt
!call setdt
!if(itime.gt.(ifirst+4))dt=dtmax*0.8
!dt0 = dt
if (nrank.eq.0)write(*,"(' dt 0,1,2=',F9.4,F9.4,F9.4)")dt0,dt1,dt2
timef=t; t=t+dt
if(itime.gt.(ifirst+3))then
call setab3(ab0,ab1,ab2)
if (nrank.eq.0)write(*,"(' AB3 0,1,2=',F9.4,F9.4,F9.4)")ab0,ab1,ab2
if (nrank.eq.0)write(*,"(' dt 0,1,2=',F9.4,F9.4,F9.4)")dt,dt1,dt2
!adt(1)=ab0*dt0
!bdt(1)=ab1*dt1
!cdt(1)=ab2*dt2
!gdt(1)=adt(1)+bdt(1)+cdt(1);gdt(3)=gdt(1)
!if (nrank.eq.0)write(*,"(' gdt(1) gdt(3)=',F9.6,F9.6)")gdt(1),gdt(3)
endif
return
end
subroutine setab3!(ab0,ab1,ab2)!Set coefficients for 3rd order Adams-Bashforth scheme (variable time step).
USE var! only : dt,dt0,dt1,dt2,itime,mytype
implicit none
!real(mytype) :: ab0,ab1,ab2,dts,dta,dtb,dtc,dtd,dte
real(mytype) :: dts,dta,dtb,dtc,dtd,dte
     ! IF (itime.eq.2) THEN           !ab2
     !    AB0 = 1.;AB1 = 0.;AB2 = 0.
     ! ELSEIF (itime.eq.2) THEN
     !    AB0 = 23./12.;AB1 = -16./12.;AB2 = 5./12.
     ! ELSE
      !AB2  = (DTLAG(1)/DTLAG(3))*((DTLAG(1)/3.+DTLAG(2)/2.)/(DTLAG(2)+DTLAG(3)))
      !AB1  = -(DTLAG(1)/DTLAG(2))*(0.5+(DTLAG(1)/3.+DTLAG(2)/2.)/DTLAG(3))
      !AB0  = 1.-AB1-AB2
...
DTD = DTS / DT1
DTE = DT0 / DTS
AB2 = DTE*(half*DTB + DTC/3._mytype)
AB1 = -half*DTA - AB2*DTD
AB0 = one - AB1 - AB2
...
if (nrank.eq.0)write(*,"(' set ab3 dt 0,1,2=',F9.4,F9.4,F9.4)")dt0,dt1,dt2
if (nrank.eq.0)write(*,"(' set ab3 ab 0,1,2=',F9.4,F9.4,F9.4)")ab0,ab1,ab2
if (nrank.eq.0)write(*,"(' set a b c dt(1) =',F9.4,F9.4,F9.4)")adt(1),bdt(1),cdt(1)
      ...
      RETURN
      END
subroutine compute_cfl(ux1,uy1,uz1)
...
implicit none
integer :: code,ierror,ijk,i,j,k
real(mytype) :: udxyzmax,udxyzmax2
real(mytype),dimension(xsize(1),xsize(2),xsize(3)) :: ux1,uy1,uz1,cfl
real(mytype),dimension(ysize(1),ysize(2),ysize(3)) :: cfl2
  ...
  !do k = 1, ysize(3)
   do j = 1, ysize(2)
  !  do i = 1, ysize(1) !cfl=dt*(ux/dx+uy/dy+uz/dz)
     cfl2(:,j,:)=dt*(abs(ux2(:,j,:)*udx)+abs(uy2(:,j,:)*udy(j))+abs(uz2(:,j,:)*udz))
     !cfl2(:,j,:)=max(abs(ux2(:,j,:)*udx),abs(uz2(:,j,:)*udz))
     !cfl2(:,j,:)=max(cfl2(:,j,:),uy2(:,j,:)*udy(j))
  !  enddo
   enddo
  !enddo
  call transpose_y_to_x(cfl2,cfl)
  courno=maxval(cfl(:,:,:))
  call MPI_REDUCE(courno,udxyzmax,1,real_type,MPI_MAX,0,MPI_COMM_WORLD,code)
  courno1=udxyzmax*dt !like this to use udxyzmax to compute dtmax

  udxyzmax2=max(udx**2 , udz**2 )
  udxyzmax2=max(udxyzmax2, maxval(udy(:))**2 )
  fourierno=udxyzmax2*dt*(xnu*fpi2)

  dtmax = min((fouriernomax/(xnu*udxyzmax2)),cournomax/udxyzmax)
  if (nrank.eq.0)then
    print *, '  CFL=',courno1,'/',cournomax
    print *, '    F=',fourierno,'/',fouriernomax
    print *, 'dtmax=',dtmax
  endif

return
end
subroutine stabiltemp !from Eric, adapted by Leonardo Romero Monteiro
  ...
  real(mytype) :: dtheta, cc, fourier, cfl
...
do is=1, nphi
 ...
 if (nrank.eq.0) then
  ...
  if (abs(phimax1).ge.10.) then !if phi control turned off
   print *,'Scalar diverged! FATALITY!'
...
if (nrank.eq.0) then
  ...
  if((abs(uxmax1).ge.10.).OR.(abs(uymax1).ge.10.).OR.(abs(uzmax1).ge.10.)) then
    print *,'Velocity diverged! FATALITY!'
    call MPI_ABORT(MPI_COMM_WORLD,code,ierror); stop
  endif
