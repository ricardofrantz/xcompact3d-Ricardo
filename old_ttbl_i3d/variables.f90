...
module var
  ...
  !real(mytype), save :: heat_flux_ant
  !real(mytype), save, allocatable, dimension(:,:) :: phi2_ant
...
contains
  ...
  subroutine init_variables
    ...
    !allocate(phi2_ant(ysize(1),ysize(3)))
    ...
    allocate(yp(ny),ypi(ny),del(ny),ypw(ny),udy(ny))
    ...
    !fourierno=max(udx**2 , udz**2 )
    !fourierno=max(fourierno,maxval(udy(:))**2 )*dt*(xnu*fpi2)
    ...
    if (nscheme.eq.2) then !AB3
       ...
       gdt(3)=gdt(1)
       cournomax=0.723_mytype/1.99_mytype
       fouriernomax=0.545_mytype/(fpi2*(pi**2))
    endif
